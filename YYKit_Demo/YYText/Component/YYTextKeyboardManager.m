...
static UIInterfaceOrientation InterfaceOrientation() {
    if (@available(iOS 13.0, *)) {
        ...
        NSArray *allWindow = [UIApplication sharedApplication].windows;
        if (!allWindow.count) {
            return UIInterfaceOrientationPortrait;
        }
        UIWindow *window = allWindow.firstObject;
        if (window.isKeyWindow && !window.isHidden) {
            keyWindow = window;
        }

        if (!keyWindow) {
            for (UIWindow *window in allWindow) {
                if (window.isKeyWindow && !window.isHidden) {
                    keyWindow = window;
                    break;
                }
...
- (void)_notifyAllObservers {
    ...
    _fromOrientation = InterfaceOrientation();
}
